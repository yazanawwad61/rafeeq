    ...
    try:
        ...
        if listings:
            ...
            for row in cursor.fetchall():
                lid, path = (row[0], row[1]) if DATABASE_URL else (
                    row['listing_id'], row['photo_path'])
                photos.setdefault(lid, []).append(path)
            cursor.execute(
                f'SELECT listing_id, tag FROM listing_tags WHERE listing_id IN ({ph})', ids)
            tags = {}
            for row in cursor.fetchall():
                lid, tag = (row[0], row[1]) if DATABASE_URL else (
                    row['listing_id'], row['tag'])
                tags.setdefault(lid, []).append(tag)
            for l in listings:
...
def update_listing(listing_id):
    try:
        ...
        cursor.execute(q('''
            ...
            gender_preference=?, rent=?, area=?, rooms=? WHERE id=?
        '''), (title, description, apartment_type, gender_preference, rent, area, rooms, listing_id))
        cursor.execute(
            q('DELETE FROM listing_tags WHERE listing_id = ?'), (listing_id,))
        for tag in tags:
